	...
	struct cam_isp_context *ctx_isp)
{
	...
	if (list_empty(&ctx->pending_req_list)) {
		ctx_isp->all_list_empty = true;
		CAM_WARN(CAM_ISP,
			"No pending request to recover from on ctx: %u all_list_empty:%d",
			ctx->ctx_id, ctx_isp->all_list_empty);
		return -EINVAL;
	}
...
move_to_pending:
	...
	if (ctx_isp->all_list_empty && ctx_isp->trigger_full_rec_cnt < CAM_ISP_CONTEXT_MAX_INTERNAL_RECOVERY_ATTEMPTS) {
		CAM_WARN(CAM_ISP, "Ignore reporting error to UMD ctx: %u, link: 0x%x error_count:%u, ",
		ctx->ctx_id, ctx->link_hdl, ctx_isp->trigger_full_rec_cnt);
		ctx_isp->trigger_full_rec_cnt++;
		rc = 0;
		goto exit;
	}
	...
	void *cmd)
{
	...
	ctx_isp->all_list_empty = false;
	ctx_isp->trigger_full_rec_cnt = 0;
